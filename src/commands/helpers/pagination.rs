...
impl PaginationState {
    ...
    pub fn create_components(&self) -> Vec<serenity::CreateActionRow> {
        let prev_btn = serenity::CreateButton::new(&self.prev_id)
            .label("◀")
            .style(serenity::ButtonStyle::Primary)
            .disabled(self.current_page == 0);

        let next_btn = serenity::CreateButton::new(&self.next_id)
            .label("▶")
            .style(serenity::ButtonStyle::Primary)
    ...
    pub fn create_disabled_components(&self) -> Vec<serenity::CreateActionRow> {
        let prev_btn = serenity::CreateButton::new(&self.prev_id)
            .label("◀")
            .style(serenity::ButtonStyle::Primary)
            .disabled(true);
...
async fn send_initial_reply<'a, F>(ctx: Context<'_>, make_embed: F, pagination_state: &PaginationState) -> Result<Option<ReplyHandle<'a>>, Error>
where
    F: Fn(usize) -> serenity::CreateEmbed + Send + Sync,
{
    if pagination_state.total_pages > 1 {
        ...
        // If there is only one page, send without button components
        ctx.send(
            poise::CreateReply::default()
                .embed(make_embed(pagination_state.current_page()))
                .ephemeral(true),
