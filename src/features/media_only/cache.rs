...
pub async fn store_media_only_channel_redis(redis: &Client, payload: &MediaOnlyChannel) -> Result<()> {
    ...
    redis.set(&key, payload_ser, Some(Expiration::EX(3600)), None, false).await.context("Failed to store media channel in cache")
}
...
pub async fn get_channel_media(data: &Data, channel_id: ChannelId) -> Result<Option<MediaOnlyChannel>> {
    ...
    let _ = store_negative_media_channel(redis, channel_id).await
...
pub async fn delete_media_only_channel(data: &Data, channel_id: ChannelId) -> Result<bool> {
    ...
    let _ = store_negative_media_channel(&redis, channel_id).await.inspect_err(|e| warn!(error = ?e, "Failed to delete media channel from cache"))
        .inspect_err(|e| warn!(error = ?e, "Failed to store media only channel in redis"));;

    Ok(true)
}

pub async fn store_media_only_channel(db: &PgPool, redis: &Client, payload: MediaOnlyChannel) -> Result<()> {
    store_media_only_in_db(db, &payload).await?;
    let _ = store_media_only_channel_redis(redis, &payload).await
        .inspect_err(|e| warn!(error = ?e, "Failed to store media only channel in redis"));
