...
async fn handle_music_command(
    ...
    match action {
        MusicAction::Pause => send_simple(&actor_tx, |respond| GuildCommand::Pause { respond }).await,
        MusicAction::Resume => send_simple(&actor_tx, |respond| GuildCommand::Resume { respond }).await,
        ...
        MusicAction::Prev => {
            ...
            send_simple(&actor_tx, move |respond| GuildCommand::Prev { vc_channel_id, respond }).await
        }
        MusicAction::Restart => send_simple(&actor_tx, |respond| GuildCommand::Restart { respond }).await,
        ...
        MusicAction::Play => {
            let query = query.ok_or_else(|| "A track name or URL is required to play.".to_string())?;
            let vc_channel_id = resolve_voice_channel(http, guild_id).await?;
...
async fn send_simple<T: serde::Serialize>(actor_tx: &Sender<GuildCommand>, build: impl FnOnce(oneshot::Sender<anyhow::Result<T>>) -> GuildCommand) -> Result<serde_json::Value, String> {
    let (respond_tx, respond_rx) = oneshot::channel();
    actor_tx
...
async fn resolve_voice_channel(http: &Arc<serenity::Http>, guild_id: serenity::GuildId) -> Result<serenity::ChannelId, String> {
    let channels = http
...
pub async fn ws_handler(
    ...
    debug!(guild_id = params.guild_id, "New WebSocket control connection");
...
async fn handle_text_message(state: &WebState, sender: &Sender<Message>, guild_id: u64, text: &str) {
    let message: Result<ClientMessage, _> = serde_json::from_str(text);

    let (request_id, action, query, requested_by_id) = match message {
        Ok(ClientMessage::Music { request_id, action, query, requested_by_id }) => (request_id, action, query, requested_by_id),
        Err(e) => {
            ...
            let _ = sender.send(Message::Text(Utf8Bytes::from(serde_json::to_string(&ack).unwrap_or_default()))).await;
            return;
        }
    };
    ...
    let ack = match reply {
        Ok(Ok(Ok(data))) => ServerMessage::Ack { request_id, ok: true, error: None, data: Some(data) },
        Ok(Ok(Err(e))) => ServerMessage::Ack { request_id, ok: false, error: Some(e), data: None },
        Ok(Err(_)) => ServerMessage::Ack { request_id, ok: false, error: Some("Music actor closed the command channel".into()), data: None },
        Err(_) => ServerMessage::Ack { request_id, ok: false, error: Some("Command timed out".into()), data: None },
    };
