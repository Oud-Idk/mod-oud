...
impl From<RawUserLevel> for UserLevel {
    fn from(r: RawUserLevel) -> Self {
        Self {
            ...
            cumulative_xp: r.cumulative_xp.cast_unsigned(),
            current_level: u32::try_from(r.current_level.cast_unsigned()).unwrap_or(u32::MAX),
            current_xp: r.current_xp.cast_unsigned(),
            username: String::new(),
        }
    }
}
    ...
    fn from(r: RawLevelReward) -> Self {
        Self {
            ...
            roles_to_add: r
                ...
                .collect(),
            remove_previous_roles: r.remove_previous_roles.unwrap_or(false),
        }
    }
}
...
pub async fn update_level(db: &PgPool, user_level: &UserLevel) -> Result<PgQueryResult> {
    let result = sqlx::query!(
        ...
        user_level.cumulative_xp.cast_signed(),
        i64::from(user_level.current_level),
        user_level.current_xp.cast_signed(),
...
pub async fn get_multipliers(db: &PgPool, guild_id: GuildId) -> Result<Vec<XpMultiplier>> {
    struct RawMultiplier {
        target_id: i64,
        target_type: String,
        multiplier: f32,
    }
    ...
    let multipliers = rows
        .into_iter()
        .filter_map(|r| {
            let target = match r.target_type.to_uppercase().as_str() {
                "CHANNEL" => XpTarget::Channel {
                    target_id: ChannelId::new(r.target_id.cast_unsigned()),
                },
                "ROLE" => XpTarget::Role {
                    target_id: RoleId::new(r.target_id.cast_unsigned()),
                },
                _ => return None,
            };
...
pub async fn upsert_level(
    ...
    let raw_cumulative_xps: Vec<i64> = cumulative_xps.iter().map(|&x| x.cast_signed()).collect();
    let raw_current_levels: Vec<i64> = current_levels.iter().map(|&l| i64::from(l)).collect();
    let raw_current_xps: Vec<i64> = current_xps.iter().map(|&x| x.cast_signed()).collect();
