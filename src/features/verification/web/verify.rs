...
pub async fn handle_verify(
    ...
    let settings = get_settings(&state.core.db, &state.core.redis, &state.core.guild_configs_cache, payload.guild_id).await
        ...
        match discord_res {
            Ok(resp) if resp.status().is_success() => {
                ...
                if discord_user.id != payload.user_id {
                    warn!("User ID mismatch! URL ID: {}, Auth ID: {}", payload.user_id, discord_user.id);
