...
pub async fn log_join_to_db(user_id: UserId, guild_id: GuildId, data: &BotData) -> Result<(), Error> {
    sqlx::query!(
...
pub async fn log_leave_to_db(user_id: UserId, guild_id: GuildId, db: &sqlx::PgPool) -> Result<(), Error> {
    sqlx::query!(
