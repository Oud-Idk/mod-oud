use crate::events::handlers::temp_voice::database::get_hub_and_cache;
...
pub(crate) async fn refresh_temp_vc_ttl(data: &Data, guild_id: GuildId, channel_id: ChannelId) -> Result<(), Error> {
...
pub async fn handle_join_hub_temp_vc(
    ...
    let maybe_hub = database::get_hub_info(guild_id, redis, db, target_channel_id, &cache_key, cached_json).await?;
    ...
    let new_channel = utils::create_temp_vc(&ctx, &guild_id, &member, &hub_info).await?;
