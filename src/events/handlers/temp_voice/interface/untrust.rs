...
pub(crate) async fn handle_untrust_temp_vc(
    ctx: &Context,
...
pub(crate) async fn handle_untrust_temp_vc_submit(
    ctx: &Context,
    interaction: &ComponentInteraction,
    data: &Data,
    ...
    for target_user_id in target_user_ids {
        debug!("Untrusting user {} in channel {}", target_user_id, channel_id);
    ...
    let content = format!("Removed {} from the trusted list.", untrusted_mentions.join(", "));
    interaction
