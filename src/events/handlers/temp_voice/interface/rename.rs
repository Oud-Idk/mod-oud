use crate::events::handlers::temp_voice::database::get_hub_info_by_category;
use crate::events::handlers::temp_voice::interface;
use crate::events::handlers::temp_voice::interface::utils::create_ephemeral_msg;
use crate::events::handlers::temp_voice::interface::{preflight_modal_check, utils};
use crate::events::handlers::temp_voice::placeholder::replace_channel_placeholders;
...
pub(crate) async fn handle_rename_temp_vc(
    ...
    let Ok(Some(_)) = interface::preflight_button_check(
...
pub(crate) async fn handle_rename_temp_vc_submit(
    ...
    let Some(new_name) = utils::get_new_name(interaction) else { return Ok(()) };
    ...
    match channel_id.edit(&ctx.http, EditChannel::new().name(
        ...
        Err(serenity::Error::Http(http_err)) if utils::is_rate_limited(&http_err) => {
