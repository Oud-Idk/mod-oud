...
use fred::interfaces::{FredResult, HashesInterface, KeysInterface, PubsubInterface, SetsInterface};
...
pub async fn update_close_button_redis(redis: &Client, ticket_key: &str, new_msg_id_i64: i64) -> Result<(), anyhow::Error> {
    let _: () = redis.hset(ticket_key, ("last_button_message_id", new_msg_id_i64)).await?;
    Ok(())
}
...
pub async fn update_activity_redis(
    ...
    let (should_rotate, last_button_id): (i32, Option<String>) = redis
        .eval(script, ticket_key, (now_ts, bump_every))
        .await?;
