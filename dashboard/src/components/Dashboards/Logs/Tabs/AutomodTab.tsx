...
export function AutomodTab({ guildId }: AutomodTabProps) {
    ...
    useEffect(() => {
        ...
        getAutomodLogs(guildId, LIMIT, null, null)
            .then((data) => {
                setLogs(data);
    ...
    const fetchMoreLogs = useCallback(async () => {
        ...
        setLoadingMore(true);
        try {
            const data = await getAutomodLogs(guildId, LIMIT, lastLogCreatedAt, lastLogId);
            if (data.length < LIMIT) setHasMore(false);
