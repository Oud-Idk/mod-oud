...
export interface FerrisProps extends SVGProps<SVGSVGElement> {
    /** Renders Ferris as a clean line-art outline instead of a solid silhouette */
    outlined?: boolean;
    /** Stroke width when `outlined` is true (default: 12) */
    strokeWidth?: number;
}

const SECONDARY_OPACITY = 0.55;

export const Crab: FC<FerrisProps> = ({
    outlined = false,
    strokeWidth = 12,
    style,
    ...props
}) => {
    const maskId = `ferris-eye-mask-${useId()}`;

    const getPathStyle = (opacity = 1): React.CSSProperties => {
        if (outlined) {
            return {
                fill: 'none',
                stroke: 'currentColor',
    ...
    return (
        <svg
            ...
            {...props}
        >
            {/* Mask that cuts real transparent holes for the eyes so whatever
                is behind the SVG (dark bg, light bg, anything) shows through.
                Only meaningful in filled mode — outlined mode has no fill to cut. */}
            {!outlined && (
                <defs>
            ...
            <g id="Layer-1">
                {/* Body group: shadows + shell/claws, masked so the eyes read as
                    transparent cutouts in filled mode. */}
                <g mask={outlined ? undefined : `url(#${maskId})`}>
                    {/* Base Shadow Layer */}
                    <g transform="matrix(1,0,0,1,597.344,637.02)">
                        <path
                            ...
                            style={getPathStyle(SECONDARY_OPACITY)}
                        />
                    </g>
                ...
                {/* Eye outlines — only drawn as visible strokes in outlined mode.
                    In filled mode, these shapes are already cut as holes above. */}
                {outlined && (
                    <>
                        <g transform="matrix(1,0,0,1,450.328,483.629)">
                            <path
                ...
                {/* Pupils: always solid currentColor dots, in both modes */}
