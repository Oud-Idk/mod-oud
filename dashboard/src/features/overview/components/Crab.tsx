...
export type FerrisVariant = 'rust' | 'white' | 'black';

export interface FerrisProps extends SVGProps<SVGSVGElement> {
    /** Color theme preset */
    variant?: FerrisVariant;
    /** Convenience shortcut for variant="white" (for dark backgrounds) */
    white?: boolean;
    /** Convenience shortcut for variant="black" (for light backgrounds) */
    black?: boolean;
    /** Renders Ferris as a clean line-art outline */
    outlined?: boolean;
...
export const Crab: FC<FerrisProps> = ({
    ...
    // Resolve active color mode
    const activeVariant: FerrisVariant =
        variant ?? (white ? 'white' : black ? 'black' : 'rust');

    // Color mappings
    const colors = {
        rust: {
            primary: 'rgb(247, 76, 0)',
            secondary: 'rgb(165, 43, 0)',
            socket: '#000000',
            pupil: '#FFFFFF',
        },
        white: {
    ...
    // Helper styles for solid vs. outlined rendering
    const getPathStyle = (fillColor: string): React.CSSProperties => {
        if (outlined) {
            return {
                fill: 'none',
    ...
    const getPupilStyle = (): React.CSSProperties => {
        if (outlined) {
            return {
                fill: colors.primary, // Fill pupil in outline mode for classic eyes
                stroke: 'none',
            };
        }
        return {
            ...
            <g id="Layer-1">
                ...
                <g transform="matrix(1,0,0,1,597.344,637.02)">
                    <path
                        ...
                        style={getPathStyle(colors.secondary)}
                    />
                </g>
                ...
                <g transform="matrix(1,0,0,1,1151.27,281.813)">
                    <path
                        ...
                        style={getPathStyle(colors.primary)}
                    />
                </g>
                ...
                <g transform="matrix(1,0,0,1,450.328,483.629)">
                    <path
                        ...
                        style={getPathStyle(colors.socket)}
                    />
                </g>
