...
export async function getLevels(guildId: string): Promise<UserLevel[]> {
    ...
    const query = `
        SELECT
            l.guild_id::text AS "guildId",
            l.user_id::text AS "userId",
            l.cumulative_xp AS "cumulativeXp",
            l.current_level AS "currentLevel",
            l.current_xp AS "currentXp",
...
export async function fetchMoreLevels(
    guildId: string,
...
export async function getLevelRewards(guildId: string): Promise<LevelReward[]> {
    ...
    const { rows } = await db.query(
        `SELECT 
            id,
            guild_id::text AS "guildId",
            level_requirement AS "levelRequirement",
            COALESCE(roles_to_add::text[], ARRAY[]::text[]) AS "rolesToAdd",
            remove_previous_roles AS "removePreviousRoles"
         FROM level_rewards 
         WHERE guild_id = $1`,
        [validGuildId]
    );
