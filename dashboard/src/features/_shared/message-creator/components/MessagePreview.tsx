...
export const MessagePreview = ({ config, plaintext, embed }: EmbedProps): JSX.Element => {
    ...
    return (
        <DiscordMessages className="rounded-md overflow-hidden" no-background lightTheme={resolvedTheme === "light"}>
            <DiscordMessage>
                <p className="whitespace-pre-wrap">{renderWithPlaceholders(plaintext)}</p>
                {(embed !== undefined) && (
                    <DiscordEmbed
                        slot="embeds"
                        color={embed.color !== "" ? embed.color : "#ffffff"}
