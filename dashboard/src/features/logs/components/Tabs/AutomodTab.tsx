...
export function AutomodTab({ guildId }: AutomodTabProps): JSX.Element {
    ...
    const lastLog: AutomodLog | undefined = logs.length > 0 ? logs[logs.length - 1] : undefined;
    ...
    useEffect(() => {
        ...
        void getAutomodLogsAction(guildId, LIMIT, null, null)
            .then((data) => {
                setLogs(data);
                if (data.length < LIMIT) setHasMore(false);
            })
            .catch((err: unknown) => {
                console.error("Error fetching initial automod logs:", err);
            })
            .finally(() => {
                setLoading(false);
            });
                    ...
                    {logs.map((log) => (
                        <TableRow key={log.id}>
                            ...
                            <TableCell className="max-w-xs truncate italic">
                                {log.trigger_content !== null && log.trigger_content !== ""
                                    ? log.trigger_content
                                    : "—"}
                            </TableCell>
